...
-- Posts: add Page/Group publishing without replacing existing app policies.
-- This is intentionally additive: existing post RLS policies remain untouched.
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname='posts_publisher_page_id_fkey'
...
-- A user can publish for their own Page or in a group where they are an active member.
DROP POLICY IF EXISTS posts_insert_page_or_group_v11623 ON public.posts;
CREATE POLICY posts_insert_page_or_group_v11623 ON public.posts
FOR INSERT TO authenticated
WITH CHECK (
  owner_id = auth.uid()
  AND (
    (
      publisher_page_id IS NOT NULL
      AND group_id IS NULL
      AND EXISTS (
        SELECT 1 FROM public.pages p
        WHERE p.id = publisher_page_id AND p.owner_id = auth.uid()
      )
    )
    OR
    (
      group_id IS NOT NULL
      AND publisher_page_id IS NULL
      AND EXISTS (
...
-- Group posts are visible only to members when the group is private.
DROP POLICY IF EXISTS posts_select_group_v11623 ON public.posts;
CREATE POLICY posts_select_group_v11623 ON public.posts
...
-- Page posts remain normal public posts unless an existing visibility policy says otherwise.
-- The publisher_page_id is protected on INSERT by the policy above.

-- Keep group member_count correct.
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname='posts_owner_id_fkey') THEN
    ALTER TABLE public.posts ADD CONSTRAINT posts_owner_id_fkey
      FOREIGN KEY(owner_id) REFERENCES public.profiles(id) ON DELETE CASCADE;
  END IF;
END $$;

-- Realtime updates should include old values for UPDATE/DELETE events.
...
-- Notify group owners/requesters from the database so notifications are real, not local demo data.
CREATE OR REPLACE FUNCTION public.tafa_group_request_notification()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  gid := COALESCE(NEW.group_id, OLD.group_id);
  SELECT g.owner_id, g.name INTO target_user, gname
  FROM public.groups g WHERE g.id = gid;

  IF TG_OP = 'INSERT' AND NEW.status = 'pending' THEN
    IF target_user IS NOT NULL AND target_user <> NEW.user_id THEN
      INSERT INTO public.notifications(user_id, actor_id, type, message, is_read, created_at)
      VALUES (target_user, NEW.user_id, 'group_join_request',
              'Nouvelle demande pour rejoindre le groupe « ' || COALESCE(gname,'') || ' ».', false, now());
    END IF;
  ELSIF TG_OP = 'UPDATE' AND OLD.status IS DISTINCT FROM NEW.status
        AND NEW.status IN ('accepted','rejected') THEN
    INSERT INTO public.notifications(user_id, actor_id, type, message, is_read, created_at)
    VALUES (NEW.user_id, target_user, 'group_join_' || NEW.status,
            CASE WHEN NEW.status='accepted'
                 THEN 'Votre demande pour rejoindre « ' || COALESCE(gname,'') || ' » a été acceptée.'
                 ELSE 'Votre demande pour rejoindre « ' || COALESCE(gname,'') || ' » a été refusée.' END,
            false, now());
  END IF;
...
DROP TRIGGER IF EXISTS trg_tafa_group_request_notification ON public.group_join_requests;
CREATE TRIGGER trg_tafa_group_request_notification
AFTER INSERT OR UPDATE OF status ON public.group_join_requests
FOR EACH ROW EXECUTE FUNCTION public.tafa_group_request_notification();
